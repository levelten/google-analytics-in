 ...
 */
class Deconfc_Logger_File extends Deconfc_Logger_Abstract
{
  /**
  ...
  public function __construct(Deconfc_Client $client)
  {
    ...
    $file = $client->getClassConfig('Deconfc_Logger_File', 'file');
    ...
    $mode = $client->getClassConfig('Deconfc_Logger_File', 'mode');
    if (!$mode) {
      $this->mode = $mode;
    }

    $this->lock = (bool) $client->getClassConfig('Deconfc_Logger_File', 'lock');
    $this->file = $file;
  }
  ...
  protected function write($message)
  {
    if (is_string($this->file)) {
      ...
      throw new Deconfc_Logger_Exception('File pointer is no longer available');
    }
